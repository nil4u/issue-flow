  ...
  return class IssueStore extends Base {
    ...
    issueFromRecord(row) {
      ...
      return {
        ...
        flow: row.flow || "",
        optimizationState: row.optimizationState || "",
        optimizationSourceIssueNumber: Number(row.optimizationSourceIssueNumber || 0),
    ...
    async upsertIssueSnapshot(input = {}, client = this.db) {
      ...
      const data = {
        ...
        author: String(input.author || existing && existing.author || ""),
        ...
        optimizationState: String(input.hasLabelSnapshot ? input.optimizationState || "" : existing && existing.optimizationState || ""),
        optimizationSourceIssueNumber: Number(input.hasDescriptionSnapshot ? input.optimizationSourceIssueNumber || 0 : existing && existing.optimizationSourceIssueNumber || 0),
