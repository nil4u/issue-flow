...
function renderCustomHtml(section, index, resources) {
  const file = text(section.file)
  const source = text(record(resources).customHtml && record(resources).customHtml[file])
  const title = text(section.title, "交互 Demo")
  const body = source
    ? `<div class="vp-demo-shell"><div class="vp-demo-toolbar"><span>交互 Demo</span><button type="button" data-vp-demo-expand aria-pressed="false">展开</button></div><iframe class="vp-demo-frame" title="${escapeHtml(title)}" srcdoc="${escapeHtml(source)}" sandbox="allow-scripts allow-forms allow-modals" referrerpolicy="no-referrer"></iframe></div>`
    : `<div class="vp-empty">Demo 页面无法加载</div>`
  return renderSectionShell(section, index, body, "界面 Demo")
}
...
function renderPlanSection(sectionValue, index, data, resources) {
  ...
  if (type === "custom-html") return renderCustomHtml(section, index, resources)
