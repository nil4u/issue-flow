...
import { drillRepositoryDashboardPanel, getDashboard, listDashboards, queryRepositoryDashboardPanel } from "../core/dashboards.js"
...
export async function dashboardRoutes(app: FastifyInstance) {
  ...
  app.post("/api/repositories/:repoId/dashboards/:slug/panels/:panelId/drill", async (request, reply) => {
    const { repoId, slug, panelId } = request.params as { repoId: string; slug: string; panelId: string }
    const result = await drillRepositoryDashboardPanel({
      ...contextFromRequest(request),
      repoId,
      slug,
      panelId,
