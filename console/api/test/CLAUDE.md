...
成员清单
agentrix-forward.test.cjs: agentrix forward 接收端 hello/events/ack 会话协议及 task usage 路由的纯逻辑测试，用 fake store，不需要数据库（真实 ws 端到端在 metrics.test.cjs）。
...
metrics-sql.test.cjs: 只读 metrics SQL 校验、参数绑定与 Issue/MR 快照解析的纯逻辑测试。
