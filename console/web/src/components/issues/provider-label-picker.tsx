...
export function ProviderLabelPicker({ labels, selected, disabled = false, onChange }: { labels: ProviderIssueLabel[]; selected: string[]; disabled?: boolean; onChange: (labels: string[]) => void }) {
  ...
  return <div className="provider-label-picker"><label><Search className="size-4" /><input value={query} onChange={(event) => setQuery(event.currentTarget.value)} placeholder="筛选 Labels…" /></label><div>{filtered.length ? filtered.map((label) => <label key={label.name} className="provider-label-option"><Checkbox checked={selectedSet.has(label.name)} disabled={disabled} onCheckedChange={(checked) => toggle(label.name, checked === true)} /><i style={{ background: labelColor(label) }} /><span><span>{label.name}</span>{label.description ? <small>{label.description}</small> : null}</span></label>) : <p>没有匹配的 Labels</p>}</div></div>
}
