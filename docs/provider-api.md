...
3. GitHub 有 token 时调用 `POST /repos/{owner}/{repo}/issues`，无 token 时 fallback 到 `gh api`
4. GitLab 创建前读取每个 managed label，缺失或颜色/说明漂移时失败并提示先运行 `sync-labels.cjs`
5. GitLab 有 token 时调用 `POST /projects/{project}/issues`，无 token 时 fallback 到 `glab api`
6. 有 `AGENTRIX_TASK_ID` 或 `--agentrix-task-id` 时，在 issue body 顶部写入 `<!-- issue-flow:agentrix:task=<id> -->`
7. 输出稳定 JSON：provider、repo、issueNumber、issueUrl、labels、dryRun
