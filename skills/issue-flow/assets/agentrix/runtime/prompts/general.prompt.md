...
- 当用户要求创建 issue，或开放讨论已经形成清晰需求时，先按 `.issue-flow/templates/type-*.md` 整理标准化正文，写到 repo 外临时文件，再使用 `issue-flow` skill 的创建标准化 issue 能力创建 provider issue。
...
- 不要手写 Agentrix task marker；创建脚本会从 `AGENTRIX_TASK_ID` 自动写入隐藏 marker。
