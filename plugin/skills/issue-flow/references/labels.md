...
Visual Plan 与 Markdown Plan 共用的审阅状态，同一 issue 最多保留一个。

| Label | 语义 |
|-------|------|
| `plan::pending` | Plan 已发布，等待审批 |
| `plan::approved` | Plan branch 已合并到默认分支 |
| `plan::changes-requested` | Plan 需要修改 |

### feature:visual-plan:

Issue 级 Plan 模式开关，与 `plan::` 审批状态使用不同 prefix，不会互相替换。
