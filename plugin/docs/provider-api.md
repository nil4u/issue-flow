...
默认或无开关时使用 Markdown Plan PR/MR；`feature:visual-plan:on` 使用 Decision/Visual Plan PR/MR；`type::optimization` 固定使用 Optimization JSON。四种产物复用同一 Plan MR 与 Engine 路由：
