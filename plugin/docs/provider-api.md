...
2. 确保 `mr-by::plan` label 存在且颜色/说明匹配 catalog（优先 token API；无 token时 fallback CLI）
...
7. Decision 设置 `decision::pending + flow::approve`；Visual Plan 设置 `visual-plan::pending + flow::approve`；Markdown Plan 设置 `flow::approve`

Engine 从 `mr-by::plan` PR/MR 发现产物。提交修改意见时，Issue Flow 使用页面当前登录用户的 OAuth token 在该 PR/MR 下评论并 `@agentrix`，由 review-comment pipeline resume 原 Plan task。批准时使用同一用户身份合并 PR/MR。Decision merge 后进入 `decision::approved + flow::plan` 并恢复原 Plan task；Visual/Markdown Plan merge 后进入 `flow::build`。
...
`type::`、`status::`、`flow::`、`decision::`、`visual-plan::`、`feature:visual-plan:`、`automation::`、`priority::`、`size::`，以及 PR/MR labels `mr-by::plan` 和 `mr-by::build`。
...
| `flow::approve` | Issue | `0E8A16` | Waiting for visual artifact or build PR/MR approval |
| `decision::pending` | Issue | `FBCA04` | Visual decision is waiting for review |
| `decision::approved` | Issue | `0E8A16` | Visual decision was approved |
| `decision::changes-requested` | Issue | `D73A4A` | Visual decision needs discussion or changes |
| `visual-plan::pending` | Issue | `FBCA04` | Visual plan is waiting for approval |
| `visual-plan::approved` | Issue | `0E8A16` | Visual plan branch was promoted to the default branch |
| `visual-plan::changes-requested` | Issue | `D73A4A` | Visual plan needs changes |
