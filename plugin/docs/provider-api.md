...
默认或无开关时使用 Markdown Plan PR/MR；`feature:visual-plan:on` 使用 Decision/Visual Plan PR/MR。三种产物统一执行前五步，审阅入口按模式分流：
...
Engine 从 `mr-by::plan` PR/MR 发现 Decision/Visual Plan JSON 产物并使用固定组件、统一布局和统一样式渲染。草稿和历史评论按 repository、issue、Decision/Plan 分区保存在浏览器 LocalStorage。提交审阅时，Issue Flow 使用页面当前登录用户的 OAuth token 在该 PR/MR 下评论，由 review-comment pipeline resume 原 Plan task。Decision 批准只评论同一个 open MR并进入 `flow::plan`，不合并；恢复后的 Plan task 更新同一分支和 MR。Visual Plan 在 Engine 批准后合并 MR；Markdown Plan 直接通过 provider MR 审阅、合并，两者合并后都进入 `flow::build`。
...
`type::`、`status::`、`flow::`、`feature:visual-plan:`、`automation::`、`priority::`、`size::`，以及 PR/MR labels `mr-by::plan` 和 `mr-by::build`。
...
`bootstrap.cjs` / `install.sh` 安装默认 CI workflow：

- GitHub 的 `.github/workflows/issue-flow-labels.yml` 会在 issue-flow 相关文件变更时自动运行 `sync-labels.cjs`。
- GitLab 由 management console 在安装检查中创建缺失 label，并更新颜色/说明漂移的 label；默认 CI 不包含 label 同步 job。
- 手工安装 GitLab plugin 后，可由具备项目 label 管理权限的用户运行 `issue-flow labels sync`。
- 如果 GitHub workflow token 没有 label 管理权限，label sync job 会失败；修复权限后重新运行 job 或再次 push 即可。
...
- 已存在文件默认跳过，`--force` 才覆盖
- GitHub 安装后的 push 会通过默认 workflow job 自动同步 provider labels；GitLab 由 management console 同步，手工安装时可运行 `issue-flow labels sync`
